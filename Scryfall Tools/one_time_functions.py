from scryfall_tools import search_for_cards


def create_set_review_lists(setcode):
    set_cards = search_for_cards(f'set={setcode} in:booster order=set')
    if not set_cards:
        print('Set not found')
        return
    cu_names = [
        (card['name'], card['collector_number']) for card in set_cards
